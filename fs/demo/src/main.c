...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    sprintf(device_path, "%s/" DEVICE_NAME, getpwuid(getuid())->pw_dir); // 输入东西到device_path
    super.driver_fd = ddriver_open(device_path);    

    printf("super.driver_fd: %d\n", super.driver_fd);
    int sz_io = 0;
    int sz_disk = 0;
    // 通过ddriver_ioctl来获取磁盘容量和IO块大小等信息。 根据传入命令cmd获取虚拟磁盘信息。cmd为IOC_REQ_DEVICE_SIZE，void *ret处返回虚拟磁盘总大小。cmd为IOC_REQ_DEVICE_IO_SZ，void *ret参数处返回虚拟磁盘的IO大小。
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &sz_io);
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &sz_disk);
    /* 填充super信息 */
    super.sz_io = sz_io;
    super.sz_disk = sz_disk;
    super.sz_blks = sz_io << 1;  //一个逻辑块是两个IO块大小
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    off_t _offset = 500 * super.sz_blks;
    ddriver_seek(super.driver_fd, _offset, SEEK_SET);

    /* TODO: 调用ddriver_read读出一个磁盘块到内存，512B */
    char _buf[512];
    ddriver_read(super.driver_fd, _buf, 512);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry dentry;
    memcpy(&dentry, _buf, sizeof(struct demo_dentry));
    /* TODO: 填充filename */
